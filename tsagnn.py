...
def write_log(message):
    with open('run/log_train.txt', 'a', encoding='utf-8') as f:
        f.write(message + '\n')
    print(message)

def save_importance_to_csv(importance_results, epoch):
    records = []
    ...
    csv_filename = f'run/importance_epoch_{epoch + 1}.csv'
...
def main():
    ...
    def train(data_list, edge_index_list, sample_names_batch, epoch):
        model.train()
        total_loss = 0
        importance_results = []
        ...
        save_importance_to_csv(importance_results, epoch)
    ...
    for epoch in tqdm(range(num_epochs), desc="Training Epochs"):
        ...
        for current_idx in range(0, len(train_data_list), batch_size):
            ...
            loss = train(data_list, edge_index_list, batch_sample_names, epoch)
            epoch_loss += loss
            num_batches += 1
    ...
    write_log(f"Test MSE: {mse:.4f}")
    write_log(f"Test RMSE: {rmse:.4f}")
    write_log(f"Test MAE: {mae:.4f}")
    write_log(f"Test MAPE: {mape:.4f}%")
    write_log(f"Test R²: {r2:.4f}")
    ...
    plt.tight_layout()
    plt.savefig('test_output_label_plot.png')
    plt.show()
