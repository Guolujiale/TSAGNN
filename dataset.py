...
class HeteroGraphDataset(InMemoryDataset):
    ...
    def process_data(self, df):
        ...
        state_encoded = df['state_encoded'].values.reshape(-1, 1)  # 保证是二维的列
        input_feature = df.iloc[:, 7:13].values  
        state_encoded = df['state_encoded'].values  # 获取 state_encoded 列
        input_feature = np.hstack((input_feature, state_encoded.reshape(-1, 1)))  # 将 state_encoded 列添加到 input_feature 中
        ...
        for business_id in unique_business_ids:
            ...
            business_state_encoded = business_df['state_encoded'].values  # 获取每个业务的 state_encoded 列
            business_input_feature = np.hstack((business_input_feature, business_state_encoded.reshape(-1, 1)))  # 将 state_encoded 添加到 input_feature 中
